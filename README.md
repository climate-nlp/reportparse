...
<h2 align="left">
    ...
    A Unified NLP Analyzer for Corporate Sustainability Reporting
</h2>
...
ReportParse is a Python-based tool designed to parse corporate sustainability or 10-K reports. 
...
- 🆕 **2024.9**: ```10k``` reader is available thanks to [sec-parser](https://github.com/alphanome-ai/sec-parser). This can read 10-K (annual report) file in the HTML format.
...
| --reader             | str ("pymupdf" or "deepdoctection")                      | The name of the PDF layout / text extraction method. The default is "pymupdf". We currently support "pymupdf", "10k", or "deepdoctection". See more detail at [Reader types](#readers).                                                                                                                                                                                                                         |
...
| ```10k```            | We use [sec-parser](https://github.com/alphanome-ai/sec-parser) to extract document structure semantics and text from a 10-K (annual report) HTML file. The input should be an HTML file not a PDF file. Note that it does not use layout analysis. We do not recognize the position of each element thus bounding boxes for all elements are always (0, 0, 0, 0). We recognize a sub-top-level section as a page and each paragraph as a block. | <ul><li>Fast</li><li>No OCR errors</li><li>Text extraction is precise</li></ul>     | <ul><li>No layout analysis</li><li>May contain some semantics parsing errors</li></ul>                          |
...
- [10k_reader.py](reportparse%2Freader%2F10k_reader.py) includes a modified code from [sec-parser](https://github.com/alphanome-ai/sec-parser/blob/main/sec_parser/processing_engine/core.py).
